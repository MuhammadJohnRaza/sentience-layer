...
if ($null -eq $gcloudCheck) {
    ...
    Write-Host "Link: https://cloud.google.com/sdk/docs/install#windows" -ForegroundColor Yellow
...
if ([string]::IsNullOrEmpty($ProjectID)) {
    Write-Host "[Warning] No GCP Project ID specified. Fetching your active gcloud project..." -ForegroundColor Yellow
...
Write-Host "`nStep 2: Setting active project configuration..." -ForegroundColor Cyan
gcloud config set project $ProjectID

# Step 5: Enable APIs
Write-Host "`nStep 3: Enabling Google Cloud APIs (this may take a minute)..." -ForegroundColor Cyan
...
Write-Host "`nStep 4: Creating Google Artifact Registry Repository..." -ForegroundColor Cyan
...
Write-Host "`nStep 5: Submitting build to Google Cloud Builds..." -ForegroundColor Cyan
...
Write-Host "Copying sentience_kernel into backend/python/ for container build..." -ForegroundColor Gray
if (Test-Path "./backend/python/sentience_kernel") {
    Remove-Item -Path "./backend/python/sentience_kernel" -Recurse -Force
}
Copy-Item -Path "./sentience_kernel" -Destination "./backend/python/sentience_kernel" -Recurse -Force

gcloud builds submit --tag $ImageTag ./backend/python

# Step 8: Provision PostgreSQL on Cloud SQL
Write-Host "`nStep 6: Checking/Creating Google Cloud SQL PostgreSQL database..." -ForegroundColor Cyan
...
Write-Host "`nStep 7: Configuring database schemas and credentials..." -ForegroundColor Cyan
# Create database
...
Write-Host "`nStep 8: Deploying Cognitive Backend to Google Cloud Run..." -ForegroundColor Cyan
...
$WsUrl = "ws://" + $ServiceUrl.Replace("https://", "") + "/ws"
...
Write-Host "Live WebSocket Telemetry: $WsUrl" -ForegroundColor Green
Write-Host "==============================================================" -ForegroundColor Green
