/** * Bias Reflection Panel */ "use client";
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <p className="text-sm text-slate-600">
          {" "}
          Real-time bias monitoring across agent decision chains. Lower is
          better.{" "}
        </p>{" "}
          ...
          {BIASES.map((bias) => (
            <div key={bias.name} className="rounded-lg border p-3">
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <Badge
                  variant={bias.status === "low" ? "default" : "secondary"}
                  className="text-xs"
                >
                  {" "}
                  {bias.status}
              ...
              <p className="text-xs text-foreground0 mt-1">
                {bias.level}% detected in recent decisions
              </p>{" "}
            </div>
          ))}
        </div>{" "}
